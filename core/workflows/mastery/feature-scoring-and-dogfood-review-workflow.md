...
4. Record reviewer, tester, and manual verdicts rather than collapsing disagreement into one sentence.
5. Assign the overall score based on the strongest level the feature fully satisfies.
6. If the score is below `5`, write explicit feedback items and route them to the existing feedback and improvement loop.
7. Emit one next action: promote, keep in internal use, route to improvement, or block.
