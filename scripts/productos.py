...
def cmd_init_workspace(args: argparse.Namespace) -> int:
    ...
    print(f"Initialized workspace from {args.template} at {args.dest}")
    return 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ProductOS next-version repo CLI.")
    parser.add_argument("--workspace-dir", type=Path, default=Path("workspaces/productos-v2"))
    ...
    init_parser.add_argument("--template", choices=["product-starter"], required=True)
