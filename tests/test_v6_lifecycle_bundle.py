...
from core.python.productos_runtime import build_v6_lifecycle_bundle_from_workspace
from core.python.productos_runtime.v6 import V6_ARTIFACT_SCHEMAS


def test_build_v6_lifecycle_bundle_from_workspace_validates(self_hosting_workspace_dir: Path):
    bundle = build_v6_lifecycle_bundle_from_workspace(
        self_hosting_workspace_dir,
        generated_at="2026-03-27T08:30:00Z",
    )

    for artifact_name, schema_name in V6_ARTIFACT_SCHEMAS.items():
...
def test_v6_lifecycle_bundle_cross_refs(self_hosting_workspace_dir: Path):
    bundle = build_v6_lifecycle_bundle_from_workspace(
        self_hosting_workspace_dir,
        generated_at="2026-03-27T08:30:00Z",
    )

    runtime_report = bundle["runtime_scenario_report_v6_lifecycle_traceability"]
    validation_report = bundle["validation_lane_report_v6_lifecycle_traceability"]
    manual_record = bundle["manual_validation_record_v6_lifecycle_traceability"]
    release_readiness = bundle["release_readiness_v6_lifecycle_traceability"]
    release_gate = bundle["release_gate_decision_v6_lifecycle_traceability"]
    ralph = bundle["ralph_loop_state_v6_lifecycle_traceability"]

    assert runtime_report["candidate_version"] == "6.0.0"
    assert runtime_report["baseline_version"] == "5.0.0"
    ...
    assert ralph["target_release"] == "v6_0_0"
