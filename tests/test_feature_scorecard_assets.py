...
def test_feature_scorecard_examples_validate(root_dir: Path):
    ...
    workspace_payload = load_json(root_dir / "workspaces" / "productos-v2" / "artifacts" / "feature_scorecard.example.json")
...
def test_workspace_manifest_wires_feature_scoring(root_dir: Path):
    manifest_path = root_dir / "workspaces" / "productos-v2" / "workspace_manifest.yaml"
...
def test_improvement_loop_tracks_feature_scorecard(root_dir: Path):
    validator = validator_for("improvement_loop_state.schema.json")
    payload = load_json(root_dir / "workspaces" / "productos-v2" / "artifacts" / "improvement_loop_state.example.json")
    errors = sorted(validator.iter_errors(payload), key=lambda item: list(item.path))
...
def test_bounded_foundation_bundle_includes_feature_scorecard(root_dir: Path):
