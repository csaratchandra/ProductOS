...
def test_feature_scorecard_examples_validate(root_dir: Path):
    ...
    workspace_payload = load_json(root_dir / "internal" / "ProductOS-Next" / "artifacts" / "feature_scorecard.example.json")
...
def test_workspace_manifest_wires_feature_scoring(root_dir: Path):
    manifest_path = root_dir / "internal" / "ProductOS-Next" / "workspace_manifest.yaml"
...
def test_improvement_loop_tracks_feature_scorecard(root_dir: Path):
    ...
    payload = load_json(root_dir / "internal" / "ProductOS-Next" / "artifacts" / "improvement_loop_state.example.json")
    errors = sorted(validator.iter_errors(payload), key=lambda item: list(item.path))
