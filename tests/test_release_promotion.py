...
def test_promote_release_from_ralph_updates_current_release_surfaces(tmp_path: Path):
    ...
    (root / "workspaces" / "productos-v2" / "docs" / "product").mkdir(parents=True)
    ...
    (root / "README.md").write_text(
        ...
        "The path name remains `productos-v2` for continuity, even though the active core line is now `4.1.0`.\n\n"
        "- V4.1 market-intelligence, Ralph-loop, readable-doc, and communication assets remain present\n",
        encoding="utf-8",
    )
    (root / "workspaces" / "productos-v2" / "docs" / "product" / "product-overview.md").write_text(
    ...
    _write_json(
        root / "workspaces" / "productos-v2" / "artifacts" / "ralph_loop_state_live_docs.example.json",
        {
            "schema_version": "1.0.0",
    ...
    overview = (root / "workspaces" / "productos-v2" / "docs" / "product" / "product-overview.md").read_text(encoding="utf-8")
    ...
    assert "active core line is now `4.2.0`" in readme
...
def test_promote_release_from_ralph_blocks_watch_level_promotion_gate(tmp_path: Path):
    root = tmp_path / "repo"
    (root / "registry" / "releases").mkdir(parents=True)
    (root / "workspaces" / "productos-v2" / "artifacts").mkdir(parents=True)
    ...
    _write_json(
        root / "workspaces" / "productos-v2" / "artifacts" / "ralph_loop_state_next_version_completion.example.json",
        {
            "schema_version": "1.0.0",
    ...
    _write_json(
        root / "workspaces" / "productos-v2" / "artifacts" / "eval_run_report.json",
        {
            "schema_version": "1.0.0",
    ...
    _write_json(
        root / "workspaces" / "productos-v2" / "artifacts" / "feature_portfolio_review.json",
        {
            "schema_version": "1.0.0",
    ...
    with pytest.raises(ValueError, match="Promotion gate is blocked"):
        promote_release_from_ralph(
            ...
            eval_run_report_path=root / "workspaces" / "productos-v2" / "artifacts" / "eval_run_report.json",
            feature_portfolio_review_path=root / "workspaces" / "productos-v2" / "artifacts" / "feature_portfolio_review.json",
        )
